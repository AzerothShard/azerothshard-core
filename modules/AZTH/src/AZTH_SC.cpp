...
public:
    ...
    bool CanAddMember(ArenaTeam* team, uint64 PlayerGuid) override
    {
        if (!team)
            return false;
...
public:
    ...
    void OnDeleteFromDB(SQLTransaction& trans, uint32 guid) override
    {
        ...
        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_ARMORY_STATS);
    ...
    bool PassedQuestKilledMonsterCredit(Player* player, Quest const* qinfo, uint32 entry, uint32 real_entry, uint64 guid) override
    {
        if (!player || !qinfo)
            return false;
    ...
    bool NotAvoidSatisfy(Player* player, AccessRequirement const* ar, uint32 target_map, bool report) override
    {
        if (!player)
            return false;
    ...
    void OnSave(Player* player) override
    {
        ...
        wowArmoryTrans->PAppend("DELETE FROM armory_character_stats WHERE guid = %u", player->GetGUIDLow());

        // Character stats
        std::ostringstream ps;
        time_t t = time(nullptr);
        ps << "INSERT INTO armory_character_stats (guid, data, save_date) VALUES (" << player->GetGUIDLow() << ", '";
    ...
    bool CanJoinInBattlegroundQueue(Player* player, uint64 /*guid*/, BattlegroundTypeId /*BGTypeID*/, uint8 joinAsGroup, GroupJoinBattlegroundResult& err) override
    {
        if (!player)
            return false;
    ...
    bool CanJoinInArenaQueue(Player* player, uint64 BattlemasterGuid, uint8 arenaslot, BattlegroundTypeId BGTypeID, uint8 joinAsGroup, uint8 IsRated, GroupJoinBattlegroundResult& err) override
    {
        if (!player)
            return false;
    ...
    bool CanSendMail(Player* player, uint64 /*receiverGuid*/, uint64 /*mailbox*/, std::string& /*subject*/, std::string& /*body*/, uint32 /*money*/, uint32 /*COD*/, Item* /*item*/) override
    {
    ...
    void PetitionBuy(Player* player, Creature* /*creature*/, uint32& charterid, uint32& cost, uint32& type) override
    {
        ...
        case ARENA_TEAM_CHARTER_2v2:
            cost = IsPvP ? 0 : ARENA_TEAM_CHARTER_2v2_COST;
            break;
        case ARENA_TEAM_CHARTER_3v3:
            cost = IsPvP ? 0 : ARENA_TEAM_CHARTER_3v3_COST;
            break;
        case ARENA_TEAM_CHARTER_5v5:
            cost = IsPvP ? 0 : ARENA_TEAM_CHARTER_5v5_COST;
            break;
        default:
            return;
        }
    ...
    void PetitionShowList(Player* player, Creature* creature, uint32& CharterEntry, uint32& CharterDispayID, uint32& CharterCost) override
    {
        ...
        {
        case ARENA_TEAM_CHARTER_2v2:
            CharterCost = IsPvP ? 0 : ARENA_TEAM_CHARTER_2v2_COST;
            break;
        case ARENA_TEAM_CHARTER_3v3:
            CharterCost = IsPvP ? 0 : ARENA_TEAM_CHARTER_3v3_COST;
            break;
        case ARENA_TEAM_CHARTER_5v5:
            CharterCost = IsPvP ? 0 : ARENA_TEAM_CHARTER_5v5_COST;
            break;
        default:
            return;
        }
...
public:
    ...
    void OnItemCreate(Item* item, ItemTemplate const* itemProto, Player const* owner) override
    {
        if (itemProto->Quality > 2 && itemProto->Flags != 2048 && (itemProto->Class == ITEM_CLASS_WEAPON || itemProto->Class == ITEM_CLASS_ARMOR))
        {
            ...
            sAZTH->GetAZTHPlayer(item->GetOwner())->CreateWowarmoryFeed(2, itemProto->ItemId, item->GetGUIDLow(), itemProto->Quality);
        }
    }
    ...
    bool CanSendAuctionHello(WorldSession const* session, uint64 guid, Creature* creature) override
    {
        if (!session)
            return false;
