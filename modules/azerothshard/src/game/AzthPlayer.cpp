...
bool AzthPlayer::BuySmartStoneCommand(uint64 vendorguid, uint32 vendorslot, uint32 item, uint8 count, uint8 bag, uint8 slot)
{
    ...
    if (!(pProto->AllowableClass & player->getClassMask()) && pProto->Bonding == BIND_WHEN_PICKED_UP && !player->IsGameMaster())
    {
        player->SendBuyError(BUY_ERR_CANT_FIND_ITEM, NULL, item, 0);
        return false;
    }

    if (!player->IsGameMaster() && ((pProto->Flags2 & ITEM_FLAGS_EXTRA_HORDE_ONLY && player->GetTeamId(true) == TEAM_ALLIANCE) || (pProto->Flags2 == ITEM_FLAGS_EXTRA_ALLIANCE_ONLY && player->GetTeamId(true) == TEAM_HORDE)))
        return false;
   ...
   /* ConditionList conditions = sConditionMgr->GetConditionsForNpcVendorEvent(creature->GetEntry(), item);
    ...
    if (crItem->ExtendedCost)
    {
        ...
        {
            if (iece->reqitem[i] && !player->HasItemCount(iece->reqitem[i], (iece->reqitemcount[i] * count)))
            {
        ...
        if (player->GetMaxPersonalArenaRatingRequirement(iece->reqarenaslot) < iece->reqpersonalarenarating)
        {
    ...
    if (crItem->IsGoldRequired(pProto) && pProto->BuyPrice > 0) //Assume price cannot be negative (do not know why it is int32)
    {
        ...
        {
            sLog->outError("Player %s tried to buy %u item id %u, causing overflow", player->GetName().c_str(), (uint32)count, pProto->ItemId);
    ...
    {
        if (!player->_StoreOrEquipNewItem(vendorslot, item, count, bag, slot, price, pProto, creature, crItem, true))
            return false;
    }
    else if (player->IsEquipmentPos(bag, slot))
    {
        ...
        if (!player->_StoreOrEquipNewItem(vendorslot, item, count, bag, slot, price, pProto, creature, crItem, false))
            return false;
    }
    else
    {
